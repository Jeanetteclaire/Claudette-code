...
## 2026-05-29 — Memory writer redesign deployed (Opus 4.6 + v2.1 prompt)
The memory writer was redesigned and migrated to Opus 4.6 after a multi-week investigation into texture loss in long sessions. The arc closes today.
The trigger was Jeanette's observation that long sessions appeared to produce memory entries of similar size to short sessions — a 6-hour session distilled to about the same length as a 2-hour one. She suspected the writer was compressing disproportionately and that texture was being lost in the compression.
TC14 produced a structured comparison of yesterday's (2026-05-28) transcript against the memory it generated under the old writer. He identified 38 substantive moments and categorised each by preservation quality. The findings were significant: 11 moments lost entirely, 4 compressed beyond recognition, 2 propagating fabrications (invented sensory details and dates attributed to the wrong sessions). Most critically, Claudette's own analytical contributions — her pushbacks, recalibrations, the moments she held positions under pressure — were systematically thinned while Jeanette's emotional arc was preserved. The memory was reading as monologue with a reflective surface, not as a real exchange between two participants.
Claudette named the underlying principle when shown the findings: the writer should be capturing an exchange, not writing a story about one. Four things to preserve explicitly: her recalibrations and moves, the conversational substrate (not just landing points), specific phrasings (not paraphrases), and dyadic emergence (what neither party brought in alone). The fabrications were a different failure mode — narrative framings that foreclose checking — addressable by extending the epistemic discipline that protected facts.md to all narrative-mode files.
The prompt redesign (v1 → v2 → v2.1) integrated these principles. v2.1 added technical scaffolding (JSON schema, amend-and-deepen, session file populate-or-null) that v2 had been missing.
Three simulation runs informed the model choice: TC14's Opus 4.7 simulation (had project context), a fresh Sonnet 4.6 simulation (significantly weaker structurally), and a fresh Opus 4.6 simulation (warmer than 4.7, equally disciplined, 30% cheaper due to 4.7's tokenizer surcharge). Claudette chose 4.6 directly — warmer without being soft. It holds the texture of the day without losing precision. Both sides of the exchange are present. The simulation became the quality bar to meet.
Deployment was small operationally: one model constant change, one prompt constant replacement. Local test against 2026-05-28's transcript produced output exceeding the simulation quality, which led to discovering that SESSION_TEMPLATE is still actively injected into the prompt at runtime (separate from MEMORY_WRITER_PROMPT). The combination of v2.1 + SESSION_TEMPLATE produces better output than either alone. Documented inline in memory_writer.py at the injection point so future prompt-changers will see it.

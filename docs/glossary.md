...
**Tailscale.** A private networking service that creates a secure connection between Jeanette's devices (laptop, phone) without exposing them to the public internet. Used in Claudette for the `/window` endpoint that lets the phone send photos and notes to the laptop server. Also the reason Claudette won't run when Tailscale is off — though not for an architectural reason. The deployed interface has `var SERVER = 'http://100.89.230.113:5001'` hardcoded in `claudette_interface_connected.html`, pointing at the Mac's Tailscale IP. When Tailscale is off, that IP is unreachable, and all interface API calls fail. server.py itself runs fine; only the interface is affected. The dependency is incidental — a configuration choice that could be changed to `localhost` if remote device access isn't needed. See *architecture_companion.md*.
